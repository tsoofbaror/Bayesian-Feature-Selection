...
def visualize_s(metadata):
    ...
    if len(feature_names) == 0:
        ...
        ax1.set_yticklabels(np.arange(1, S.shape[1] + 1), fontsize=label_size)
    else:
        ax1.set_yticks(range(S.shape[1]))
        ax1.set_yticklabels(feature_names[:S.shape[1]], fontsize=label_size)
    ...
    if len(feature_names) == 0:
        ...
        ax2.set_yticklabels(np.arange(1, S.shape[1] + 1), fontsize=label_size)
    else:
        ax2.set_yticks(range(S.shape[1]))
        ax2.set_yticklabels(feature_names[:S.shape[1]], fontsize=label_size)
    ...
    ax1.imshow(S.T, cmap=cmap, aspect='auto')
    ax2.imshow(sorted_S.T, cmap=cmap, aspect='auto')
...
def geweke_plot(geweke_pairs):
    ...
    names = ['Y', "Sigma", "Tau", "a", "Number of S's", 'W_1', 'W_2']
    fig, axes = plt.subplots(nrows=len(names), ncols=2, figsize=(8, 10))
    throw_away = 0.3
    font_size = 10
